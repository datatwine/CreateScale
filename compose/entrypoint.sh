...
# 4) Start Gunicorn as the unprivileged user (proper signal handling via exec)
exec su -s /bin/sh -c "exec gunicorn myproject.wsgi:application --bind 0.0.0.0:8000 --workers 3" appuser
