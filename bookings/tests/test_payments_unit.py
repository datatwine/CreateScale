...
class TestCreateOrder:
    ...
    def test_raises_if_performer_not_kyc_approved(
        self, engagement, mock_razorpay
    ):
    ...
    def test_raises_if_performer_has_no_linked_account(
        self, engagement, mock_razorpay
    ):
...
class TestVerifyAndCapture:
    ...
    def test_happy_path_captures_payment(self, engagement, settings):
        ...
        result = PaymentService.verify_and_capture(
            "order_X", "pay_Y", sig
        )
    ...
    def test_rejects_invalid_signature(self, engagement, settings):
        ...
        with pytest.raises(ValueError, match="Invalid signature"):
            PaymentService.verify_and_capture(
                "order_X", "pay_Y", "totally_bogus_sig"
            )
    ...
    def test_rejects_when_terminal_state(self, engagement, settings):
        ...
        Payment.objects.create(
            engagement=engagement, amount=2000,
            razorpay_order_id="order_X", status="refunded",
        )
        sig = self._valid_signature("order_X", "pay_Y", "test_secret")
...
class TestReleaseToPerformer:
    ...
    def test_releases_held_transfer(self, engagement, mock_razorpay):
        ...
        mock_razorpay.transfer.edit.assert_called_once_with(
            "trf_ABC", {"on_hold": 0}
        )
        engagement.refresh_from_db()
...
class TestWebhookRouter:
    ...
    def test_payment_captured_routes_to_capture_handler(
        ...
        monkeypatch.setattr(
            PaymentService, "mark_captured_from_webhook", fake_capture
        )

        PaymentService.handle_webhook_event({
            "event": "payment.captured",
            "payload": {"payment": {"entity": {
                "id": "pay_Y", "order_id": "order_X",
            }}},
        })
