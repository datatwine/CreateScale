...
class TestGatewayWebhookIntegration(TestCase):
    ...
    def test_payment_failed_marks_created_row_failed(self):
        # M2: a gateway attempt failed. The open "created" row must flip to
        # "failed" so create_order mints a fresh order next time instead of
        # resuming a dead one.
        self.assertEqual(self.payment.status, "created")
        resp = self._post(
            "payment.failed",
    ...
    def test_unknown_event_type_returns_200_no_crash(self):
        # A genuinely unhandled event (order.paid, or a brand-new event
        # Razorpay adds) — log and ack with 200 so Razorpay stops retrying,
