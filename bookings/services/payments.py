...
class PaymentService:
    ...
    def create_order(engagement: Engagement) -> dict:
        """
        Creates a Razorpay Order with a HELD transfer to the performer's
        linked account. Returns {order_id, amount, currency, key_id} for the
        frontend checkout.js to consume.

        The transfer's on_hold=1 flag keeps the performer's share in escrow
        inside Razorpay's vault until release_to_performer() unholds it.
        ArtKhoj's 5% platform share lands in our balance immediately.
        """
        ...
        if (not performer_profile.razorpay_account_id
                or performer_profile.razorpay_kyc_status != "approved"):
            raise ValueError(
        ...
        performer_share_paise = performer_share * 100
    ...
    def release_to_performer(engagement: Engagement) -> None:
        """
        Unholds the Razorpay transfer so the performer's bank gets the money.
        Called by Celery 24h after event end. Idempotent. Skipped if the
        engagement was disputed by the client — those wait for admin action.
        """
    ...
    def refund_to_client(engagement: Engagement) -> None:
        """
        Full refund with auto-reversal of held transfer. reverse_all=1 tells
        Razorpay to cancel the linked transfer too — so the performer's
        escrowed share unwinds back to the client, no manual reconciliation.
        Idempotent.
        """
        if engagement.payment_status != Engagement.PAYMENT_PAID:
            return
